            ...
            .then(() => {
                chrome.storage.local.get(["meetings"], function (resultLocalUntyped) {
                    ...
                    chrome.storage.sync.get(["webhookUrl", "autoPostWebhookAfterMeeting"], function (resultSyncUntyped) {
                        ...
                        // Promise to save the pending transcript for our new UI
                        promises.push(postToAnalyzerBackend(lastIndex));
    ...
    return new Promise((resolve, reject) => {
        chrome.storage.local.get(["meetings"], function (resultLocalUntyped) {
            ...
            if (resultLocal.meetings && resultLocal.meetings[index]) {
                ...
                const fileName = `Meeting Conversational Assistant/Transcript-${sanitisedMeetingTitle} at ${formattedTimestamp}.txt`
                ...
                // Add branding
                content += "\n\n---------------\n"
                content += "Transcript generated by Meeting Conversational Assistant"
                content += "\n---------------"
                    ...
                    if (event.target?.result) {
                        ...
                        chrome.downloads.download({
                            ...
                            chrome.downloads.download({
                                // @ts-ignore
                                url: dataUrl,
                                filename: "MeetingConversationalAssistant/Transcript.txt",
                                conflictAction: "uniquify"
                            })
                            console.log("Invalid file name. Transcript downloaded to directory with simple file name.")
...
function postToAnalyzerBackend(index) {
    const COMPLETED_SESSION_KEY = "activeSession"; // Key for a fully analyzed session
    const PENDING_TRANSCRIPT_KEY = "pendingTranscript"; // Key for a raw transcript waiting for analysis
    ...
    return new Promise((resolve, reject) => {
        chrome.storage.local.get(["meetings"], function (resultLocalUntyped) {
            ...
            if (!resultLocal.meetings || !resultLocal.meetings[index]) {
                return reject(new Error("Pending Save: Meeting at specified index not found."));
            }

            const meeting = resultLocal.meetings[index];

            // 1. Format transcript data using existing helper functions
            const transcriptString = getTranscriptString(meeting.transcript);
            const chatString = getChatMessagesString(meeting.chatMessages);
            const fullTranscriptText = `TRANSCRIPT:\n\n${transcriptString}\n\n---------------\nCHAT MESSAGES\n---------------\n\n${chatString}`;

            // 2. Clear any OLD completed session first.
            chrome.storage.local.remove(COMPLETED_SESSION_KEY, () => {
                
                // 3. Now, save the NEW raw transcript as "pending".
                chrome.storage.local.set({ [PENDING_TRANSCRIPT_KEY]: fullTranscriptText }, () => {
                    console.log("Saved pending transcript to storage and cleared old active session.");
                    resolve("Pending transcript saved.");
                });
            });
        });
    });
